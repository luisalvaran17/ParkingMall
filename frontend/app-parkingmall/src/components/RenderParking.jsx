...
class RenderParking extends Component {
  ...
  randomGenerator() {
    ...
    for (let i = 0; i < this.state.carMatrix.length; i++) {
      for (let j = 0; j < this.state.carMatrix.length * 3; j++) {
        if (
          (i === 0 && j === 0) ||
          (i === 0 && j === 1) ||
          (i === 1 && j === 0) ||
    ...
    return this.updateState(this.state.carMatrix);
  }
  ...
  updateState(matrixUpdate) {
    ...
    let carListLocal2 = this.setListCars(listLabels[1], "A", 13);
    let carListLocal3 = this.setListCars(listLabels[2], "A", 25);
    let carListLocal4 = this.setListCars(listLabels[3], "A", 37);
